...
def multiAE(dims, dcec_input_shape=(28, 28, 1), filters1=[1, 6, 20, 60, 256, 10], filters2=[32, 64, 128, 10]):
    ...
    # Fusion
    fusion = FusionLayer(name='fusion')([encoder_4, dcec_embedding])
    ...
    return Model(inputs=[sae_input, dcec_input], outputs=[decoder_1, deconv_1], name='AE'), \
           Model(inputs=[sae_input, dcec_input], outputs=fusion, name='encoder'), \
           Model(inputs=[sae_input, dcec_input], outputs=[encoder_4, dcec_embedding], name='embedding')
...
class MDEC(object):
    ...
    def compile(self, loss=['kld', 'mse', 'mse', 'mse'], optimizer='adam'):
        ...
        #     1 + self.cvae_log_var - K.square(self.cvae_mean) - K.exp(self.cvae_log_var), axis=-1) * (-0.5)
        cae_loss = K.mean((self.dcec_input - self.deconv_1) ** 2)
        loss1 = KLD + sae_loss  + cae_loss
        # self.model.add_loss(loss1)
        self.model.compile(loss=['kld', 'mse', 'mse'], optimizer=optimizer)
        ...
        for ite in range(int(maxiter)):
            ...
            if (index + 1) * batch_size > x1.shape[0]:
                loss = self.model.train_on_batch(
                    x=[x1[index * batch_size::], x2[index * batch_size::]],
...
if __name__ == "__main__":
    ...
    mdec.fit(x=[x1, x2], y=y, tol=args.tol, maxiter=args.maxiter,
