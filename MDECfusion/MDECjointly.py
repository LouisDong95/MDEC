...
from keras.layers import Dense, Input, Lambda, Concatenate, Add
...
class FusionLayer(Layer):
    ...
    def build(self, input_shape):
        self.beta = self.add_weight(name='beta', shape=(3, 1), initializer='uniform', trainable=True, constraint=lambda beta: tf.clip_by_value(beta, 0, 1))
        self.built = True

    def call(self, inputs, **kwargs):
        z = (self.beta[0] * inputs[0] + self.beta[1] * inputs[1] + self.beta[2] * inputs[2]) / K.sum(self.beta[0] + self.beta[1] + self.beta[2])
        # self.add_loss(tf.math.squared_difference(z, (self.beta[0] * inputs[0] + self.beta[1] * inputs[1] + self.beta[2] * inputs[2])))
        return z
...
def multiAE(dims, dcec_input_shape=(28, 28, 1), filters1=[1, 6, 20, 60, 256, 10], filters2=[32, 64, 128, 10]):
    ...
    decoder_4 = Dense(dims[3], activation='relu', name='decoder_4')(encoder_4)
    ...
    #     #AE1
    # encoder_1 = Dense(dims[1], activation='relu', name='encoder_1')(sae_input)
    # decoder_1 = Dense(dims[0], name='decoder_1')(encoder_1)
    #     #AE2
    # encoder_2 = Dense(dims[2], activation='relu', name='encoder_2')(encoder_1)
    # decoder_2 = Dense(dims[1], activation='relu', name='decoder_2')(encoder_2)
    #     #AE3
    # encoder_3 = Dense(dims[3], activation='relu', name='encoder_3')(encoder_2)
    # decoder_3 = Dense(dims[2], activation='relu', name='decoder_3')(encoder_3)
    #     #AE4
    # encoder_4 = Dense(dims[4], name='encoder_4')(encoder_3)
    # decoder_4 = Dense(dims[3], activation='relu', name='decoder_4')(encoder_4)
    ...
    x2 = Dense(units=filters2[2] * int(dcec_input_shape[0] / 8) * int(dcec_input_shape[0] / 8), activation='relu')(
        dcec_embedding)
    ...
    # embedding = Concatenate(axis=1)([encoder_4, z, dcec_embedding])
    # embedding = Add()([encoder_4, z, dcec_embedding])
    embedding = FusionLayer(name='fusion')([encoder_4, z, dcec_embedding])
    return Model(inputs=[sae_input, conv_vae_input, dcec_input], outputs=[decoder_1, dcvae_1, deconv_1], name='AE'), \
           Model(inputs=[sae_input, conv_vae_input, dcec_input], outputs=embedding, name='encoder')
...
class MDEC(object):
    def __init__(self,
        ...
        self.multiAE, self.encoder = multiAE(dims, dcec_input_shape, filters1, filters2)
    ...
    def pretrain(self, x1, x2, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp'):
        ...
        loss = sae_loss + conv_vae_loss + cae_loss
        ...
        # begin training
        t0 = time()
        self.multiAE.fit([x1, x2, x2], batch_size=batch_size, epochs=epochs, callbacks=[csv_logger])
    ...
    def extract_feature(self, x1, x2):  # extract features from before clustering layer
        return self.encoder.predict([x1, x2, x2])

    def predict(self, x1, x2):
        q, _, _, _= self.model.predict([x1, x2, x2], verbose=0)
        return q.argmax(1)
    ...
    def compile(self, loss=['kld', 'mse', 'mse', 'mse'], optimizer='adam'):
        ...
        sae_loss = K.mean((self.sae_input - self.decoder_1)**2)
        conv_vae_loss = K.mean((self.dcvae_1 - self.conv_vae_input)**2) + K.sum(1 + self.cvae_log_var - K.square(self.cvae_mean) - K.exp(self.cvae_log_var), axis=-1) * (-0.5)
        cae_loss = K.mean((self.dcec_input - self.deconv_1)**2)
        loss1 = KLD + sae_loss + conv_vae_loss + cae_loss
        # self.model.add_loss(loss1)
        self.model.compile(loss=['kld', 'mse', 'mse', 'mse'], optimizer=optimizer)

    def fit(self, x1, x2, y=None, batch_size=256, maxiter=2e4, tol=1e-3,
        ...
        if not self.pretrained and multiAE_weights is None:
            ...
            self.pretrain(x1, x2, batch_size, epochs=200, save_dir=save_dir)
            self.pretrained = True
        ...
        kmeans = KMeans(n_clusters=self.n_clusters, n_init=20, random_state=50)
        self.y_pred = kmeans.fit_predict(self.encoder.predict([x1, x2, x2]))
        hidden = self.encoder.predict([x1, x2, x2])
        ...
        for ite in range(int(maxiter)):
            ...
            if (index + 1) * batch_size > x1.shape[0]:
                loss = self.model.train_on_batch(x=[x1[index * batch_size::], x2[index * batch_size::], x2[index * batch_size::]],
                                                 y=[p[index * batch_size::], x1[index * batch_size::], x2[index * batch_size::], x2[index * batch_size::]])
                index = 0
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='results/mdecjointly')
    ...
    mdec = MDEC(dims=[x1.shape[-1], 500, 500, 2000, 10], dcec_input_shape=x2.shape[1:], filters1=[1, 6, 20, 60, 256, 10], filters2=[32, 64, 128, 10], n_clusters=args.n_clusters)
    ...
    mdec.fit(x1, x2, y=y, tol=args.tol, maxiter=args.maxiter,
