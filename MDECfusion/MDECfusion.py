...
class FusionLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        fusion = (self.beta[0] * inputs[0] + self.beta[1] * inputs[1] + self.beta[2] * inputs[2]) / K.sum(
...
def multiAE(dims, dcec_input_shape=(28, 28, 1), filters1=[1, 6, 20, 60, 256, 10], filters2=[32, 64, 128, 10]):
    ...
    cvae_1 = Conv2D(filters1[0], 2, strides=2, padding='same', activation='relu', name='cvae_1')(conv_vae_input)
    cvae_2 = Conv2D(filters1[1], 2, strides=2, padding='same', activation='relu', name='cvae_2')(cvae_1)
    cvae_3 = Conv2D(filters1[2], 3, strides=2, padding='same', activation='relu', name='cvae_3')(cvae_2)
    cvae_4 = Conv2D(filters1[3], 3, strides=2, padding='same', activation='relu', name='cvae_4')(cvae_3)
    x1 = Flatten()(cvae_4)
    ...
    # Fusion
    fusion = FusionLayer(name='fusion')([encoder_4, z, dcec_embedding])
    ...
    dcvae_4 = Conv2DTranspose(filters1[2], 3, strides=2, padding='same', activation='relu', name='dcvae_4')(x1)
    dcvae_3 = Conv2DTranspose(filters1[1], 3, strides=2, padding='same', activation='relu', name='devae_3')(dcvae_4)
    dcvae_2 = Conv2DTranspose(filters1[0], 2, strides=2, padding='same', activation='relu', name='devae_2')(dcvae_3)
    dcvae_1 = Conv2DTranspose(dcec_input_shape[2], 2, strides=2, padding='same', activation='relu', name='dcvae_1')(dcvae_2)
    ...
    return Model(inputs=[sae_input, conv_vae_input, dcec_input], outputs=[decoder_1, dcvae_1, deconv_1], name='AE'), \
           Model(inputs=[sae_input, conv_vae_input, dcec_input], outputs=fusion, name='encoder'), \
           Model(inputs=[sae_input, conv_vae_input, dcec_input], outputs=[encoder_4, z, dcec_embedding], name='embedding')
...
class MDEC(object):
    ...
    def predict(self, x):
        q, _, _, _ = self.model.predict(x, verbose=0)
        return q.argmax(1)
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e4, tol=1e-3,
        ...
        import pandas as pd
        tsne = TSNE()
        data = tsne.fit_transform(self.encoder.predict(x))
        ...
        plt.savefig(save_dir + '/KMeans.png')
        return
        ...
        for ite in range(int(maxiter)):
            ...
            if (index + 1) * batch_size > x1.shape[0]:
                loss = self.model.train_on_batch(
                    x=[x1[index * batch_size::], x2[index * batch_size::], x2[index * batch_size::]],
                    y=[p[index * batch_size::], x1[index * batch_size::], x2[index * batch_size::],
                       x2[index * batch_size::]])
                index = 0
            else:
...
if __name__ == "__main__":
    ...
    mdec.fit(x=[x1, x2, x2], y=y, tol=args.tol, maxiter=args.maxiter,
