...
class FusionLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        level_weights = K.concatenate([inputs[0], inputs[1], inputs[2]], 0)
        level_weights = tf.keras.activations.softmax(level_weights, 0)
        # fusion = (self.beta[0] * inputs[0] + self.beta[1] * inputs[1] + self.beta[2] * inputs[2]) / K.sum(
        #     self.beta[0] + self.beta[1] + self.beta[2])
        fusion = inputs[0] * level_weights[0:1, :] + inputs[1] * level_weights[1:2, :] + inputs[2] * level_weights[2:3, :]
        self.add_loss((fusion - inputs[0] * level_weights[0:1, :])^ 2 + (fusion - inputs[1] * level_weights[1:2, :])^ 2 + (fusion - inputs[2] * level_weights[2:3, :])^ 2)
        return fusion
...
class MDEC(object):
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e4, tol=1e-3,
        ...
        if not self.pretrained and multiAE_weights is None:
            ...
            self.pretrain(x, batch_size=256, epochs=200, save_dir=save_dir)
            self.pretrained = True
